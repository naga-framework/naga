...
log_level() -> log_level(lager_console_backend).
log_level(Backend) -> lager:get_loglevel(Backend).

log_modules() -> wf:config(naga,log_modules).
...
stop() -> application:set_env(naga, log_modules, []).

set_modules(any) -> application:set_env(naga, log_modules, any);
set_modules(List) when is_list(List) -> application:set_env(naga, log_modules, List).
    
set_loglevel(Level) -> lager:set_loglevel(lager_console_backend, Level).
